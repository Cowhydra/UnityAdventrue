...
public class MiddleHpAttack : Behavior_Node
{
    ...
    public MiddleHpAttack(Transform transform)
    {
    ...
    public override Define.Behavior_NodeState Evaluate()
    {
        ...
        if (_attackCounter >= _attackTime)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    GameObject ESkill = Managers.Resource.Instantiate($"ESkill_{transform.gameObject.name}");
                    ESkill.GetOrAddComponent<ESkill_1030>();
